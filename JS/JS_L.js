...
document.querySelector(".divclassforMouse_6").onmousemove = () => {
  document.querySelector(".divclassforMouse_6").style.height =
    mouseheight + "px";
  mouseheight++;
};
...
document.querySelector(".arrayTrain_1").innerHTML =
  "Только чётные из array_19 - " + varForArray_19; // только четные 2_4_
...
let finall = "f";
try {
  // это просто попытка выполнить блок кода он может либо выполниться либо нет
  document.querySelector(".suadfh").innerHTML = aa;
} catch (err) {
  // после try у нас идёт catch
...
// _________________________________ 9 минут про чистые функции ______________________________________

/* 

Чистая функция это функция которая соответствует одному критерию и одному следствию из критерия

Если мы вводим одни и те же параметры, то чистая функция всегда возвращает один и тот же результат,
То есть соответствие между входными и выходными параметрами всегда 100%

function PureFunction (parametr) {
    // функция делает какие-то действия и
    return parametr*5
} - это пример чистой функции 

При одних и тех же параметрах parametr у меня функция всегда будет возвращать один и тот же результат
...
сколько бы раз я ее не запускал с параметром 5 на выходе всегда будет 25 - чистая функция
Это первый критерий чистой функции

Второй критерий - отсутствие side эффект (боковые побочные эффекты дополнительные эффекты краевые эффекты)

side эффекты - это эффекты которые производит функи при своей работе

это и AJAX запросы и вывод на страницу вывод консоль логов


Если ваша функция в ходе работы изменяет состояние системы и система становится другой
не такой какой она была до запуска функции то у нас есть side эффекты и тогда функция не может называться чистой

Пример функциия которая не является чистой
...
я совершил вывод, изменил состояние системы по определению такая функция не является чистой
это грубый пример side эффекта

Чистая функция это та функция которая принимает парамер и всегда возвращает один и тот же результат
в зависимости от этого параметра
Запустив ее любое количество раз я могу быть уверен что получу один и тот же ответ

Чистые функции обладают несколькими полезными свойствами:

1. Позволяют потенциально уменьшить количество багов
Если функция чистая значит я могу быть уверен что эта функция в системе ничего не изменяет
А значит максимально низко влияет на остальную систему
Я могу быть уверен что если баг в функции то он внутри нее он не пойдет гулять по системе
Я могу быть уверен что если функция работает корректно то она не влияет на систему
Это позволяет потенциально уменьшить количество ошибок

2. Такую функцию легче тестировать
Систему в которой есть чистые функции легче тестируются это второй плюс
Я могу обойтись одним тестом

if (parametr(5) === 25) {
    работает корректно
}

2-3 таких значения и полностью уверен что функция протестирована
Соответственно я могу выкинуть ее из дальнейшего тестирования
И заняться тестированием системы с экономив время

3. Правильно написанную чистую функцию легче понимать, то есть она позволяет легче ее понять
Поскольку все что она делает заключено внутри нее не нужно гонять по системе не нужно смотреть зависимости
Не нужно идти по дереву вызовов

4. Это ускоряет работу между программистами и программиста с системой
...
function d (b) {
    return b*c
} - пример классической функции которая зависит от внешних данных

соответственно при ее вызове 
console.log(d(5)) 45
но стоит изменить c и вызвать функцию я уже получу другой результат
c = 88 
console.log(d(5)) 440

Чистые функции подходят для каких-то отдельных видов действий типов вычислений и нельзя систему построить всю на чистых функциях
все равно придется что-то выводить что-то менять придется изменять состояние каих-то входных данных
поэтому чистая функция не является панацеей 
...
// __________________________________ Точка или квадратные скобки в объекте JavaScript __________________________

/* 

const a = [1,2,3]

Для того чтобы обратиться к массиву мы используем так называемый индекс или иногда говорят ключ

если я хочу увидеть 3 
...
для того чтобы обратиться к элементу объекта мне нужно указать ключ g 1 p
console.log(object_1.g) - 8
после object_1.g происходит поиск внутри object_1 

Есть нюанс если значение ключа лежит в переменной 

let variable_1 = 'g'

Если я попытаюсь обратиться таким образом
console.log(object_1.variable_1) undefined

.variable_1 часть ищется как ключ в объекте object_1 такого ключа нет - undefined

для того чтобы JS воспринял это как переменную мне необходимо поступить следующим образом 
использовать []

console.log(object_1[variable_1])

для объекта если мы пишет object_1.variable_1 то это воспринимается как ключ

если у нас ключ лежит в переменной допустим мы его получили из input то обазательно квадратные скобки
[ переменная ] - ключ в переменнной

так же можно указывать в [значение переменной в кавычках]
console.log(object_1['g'])

К элементу объекта можно обратиться по ключу ключ может быть указан явно
двумя способами

1. способ

явное указание ключа через точку - console.log(object_1.variable_1)

2. способ ключ лежит в переменной переменная просто в [] console.log(object_1[variable_1])

3. способ название ключа содержит пробел и написать его через точку не получится console.log(object['ilia k'])

4. обращение к значению ключа способ console.log(object_1['g'])  

я могу положить значение ключа в переменную и обратиться через переменную но тогда
обязательно квадратные скобки
я могу обратиться всегда через квадратные скобки но если я указываю ключ явно я должен взять его в кавычки


const object_1 = {
    g:8, 1:12, p:223, 'ilia k': 18
} // ключи можно писать без кавычек если они не содержат пробелов

если я вызову

console.log(object.ilia k) то в object будет искаться ключ ilia а k воспримется как некая переменная которая даст нам в итоге ошибку
но и  
console.log(object.'ilia k') - нельзя такого синтаксиса не существует
поэтому единственный способ обращения через ['']
console.log(object.['ilia k'])

существует смешанный синтаксис

const object_1 = {
    g:8, 1:12, p:223, 'ilia k': 18, t : [88,66,44]
}

console.log(object_1.t) а дальше нужно указать индекс
console.log(object_1.t[1])

// но можно и по другому

console.log(object_1['t'][1]) // 66

// пусть t получено из инпута в переменную y
...
console.log(object_1[variable_1]); // 8

console.log(object_1["g"]); // 8

console.log(object_1["ilia k"]); // 18

// const object_1 = {
//     g:8, 1:12, p:223, 'ilia k': 18, t : [88,66,44]
// } для обращения к t можно указать следующий синтаксис

console.log(object_1.t[1]); // 66

// но можно и по другому

console.log(object_1["t"][1]); // 66 [][] - вложение

// пусть t получено из инпута в переменную y

let y = "t";

console.log(object_1[y][1]); // 66

/* 

Итог:

1. Если через точку то надо явно указать ключ
2. Если ключ лежит в переменной или получает из инпут . не подходит только []
можно забыть о точке и писать только через ['t'] указываю ключ как строку в кавычках внутри этих []

*/

// _____________________________ "Готовим" стрелочные функции JavaScript. Небольшие хитрости_______________________
...
console.log(f_3.name); // f_3 - так можно получить имя функции

// если я работаю с анонимной функцией есть 2 нюанса

// 1 позволяет мне присвоить переменной стрелочную функцию
...
Когда я использую именованые функции проблем с дебагингом не возникает особенно когда у меня внутри одной функции 
еще одна функция, но как только я использую анонимную функцию

пример со стрелочными функциями в анонимном стиле

при клике вызывается вот эта функция
() => {
        count++
        console.log(count)
    }, 1000 ) // 1 секунда
}) 
параметр тоже является стрелочной функцией

чтобы не запутаться во вложенности нужно дать функциям имена
*/
/* 
let count = 0
...
}) // при клике на кнопку запускается анонимная стрелочная функция


вносим изменения в анонимную функцию
const increment = который принимает параметр число n и возвращает n + 1

const increment = n => n +1

n - имя функции

теперь я могу этот increment засунуть внутрь функции
то есть у меня будет инкремент который будет выполнять данное действие

пусть у меня increment будет делать следующее
...
итак я написал стрелочную функцию которая заменяет внутренности другой стрелочной функции выше
поэтому могу пользоваться ее именем

document.querySelector('.zaebutton').addEventListener('click', () => {
    setTimeout(increment, 1000 ) // 1 секунда
})

но у меня осталась еще одна функция которая должна срабатывать при клике
...
/* но даже использование стрелочных функций с именем имеет свои нюансы

const array_46 = [1,2,3]

с помощью map идеально использовать стрелочные функции в качестве callback

const array_46 = [1,2,3]

let array_47 = array_46.map(item => item*3)

когда у меня один параметр не обязательно писать () когда у меня return в одну строку мне необязательно писать
return {}

нюанс

напишем функцию которая принимает число и возвращает число
...
/* Напишем эту же функцию в виде стрелочной */
...
// классно но сложно рефакторить поэтому мы можем само выражение взять в () просто для лучшей читаемости
// const z_2 = n => (n >= 0 ? n : 0)

/* 

круглые скобки помогают сделать еще одну вещь
допустим есть массив
надо массив array_49 с помощью метода map и стрелочной функции преобразовать его в массив вложенных объектов
let array_50 = array_49.map(item => {'num' : item}) - ругается
если я хочу возвратить объект объект находит в {} но
из за фигурных скобок стрелочная функция начинает думать что это у меня синтаксис оборачивания фигурными скобками
стрелочной функции item => {'num' : item} и поэтому должно быть разделение через ;
но если мы обернем {'num' : item} в () то проблема решится

const array_49 = [2,3,5]

let array_50 = array_49.map(item => ({'num' : item}))
console.log(array_50);
...
document.querySelector(".zaebutton").addEventListener("click", clickHandle);
...
// Задача сделать такой же но без дубликатов

/* 
1. способ set - уникальная конструкция которая содержит только уникальные элементы
*/

let variable_2 = new Set(array_52);
...
но это набор а нужен массив
чтобы перевести набор в массив есть 2 способа
1 спред
2 Array.from
*/

// const array_52 = [1,1,2,2,3,9,3,4,4,5,5,6,6,7,7,8,8,9]
...
let variable_4 = Array.from(new Set(array_52));
console.log(variable_4); // (9) [1, 2, 3, 9, 4, 5, 6, 7, 8]

// 3 way to make all with filter

// filter accepts a few parameters, and don't forget the callback
// item - is an element that is being processed at the moment
// index - is a number in the array

/*
у меня есть исходный массив (18) [1, 1, 2, 2, 3, 9, 3, 4, 4, 5, 5, 6, 6, 7, 7, 8, 8, 9]
с помощью фильтер я поочереди получаю каждый элемент в item, 
получаю допустим элемент 1 под индексом 0

let variable_5 = array_52.filter((item(1(0)), index) => {
    return array_52.indexOf(item(1(0)) === index(1(0) -> true
}) 

беру исходный массив и проверяю indexOf элемента 1 нахожу его индекс 0 
сверяю этот индекс и тот индекс который первым найден в массиве, 

indexOf всегда ищет вначале массива! 

как только нашел возвращает позицию элемента
если позиция текущего элемента совпадает с найденым значит он уникальный и
можно интерпретировать как true  - array_52.indexOf(item) === index - и добавить в новый массив
его можно добавить в новый массив

let variable_5 = array_52.filter((item(1[0]), index1[0]) => {
    return array_52.indexOf(item(1[0]) === index(1[0] -> true
}) 


но если я найду элемент 1 под индексом 5 то в перебираемом массиве индекс будет 5

a indexOf всегда ищет вначале массива! 

indexOf найдет элемент 1 в начале массива с индексом 0 индексы 0 и 5 не совпадут значит можно сделать вывод 
что это дубляж
и такой элемент не добавится

let variable_5 = array_52.filter((item(1[5]), index(1[5]) => {
    return array_52.indexOf(item(1[0]) === index(1[5] -> false
}) 
...
то получу массив дубликатов (6) [1, 3, 5, 7, 8, 9]
...
// 4 way to make it with reduce
// reduce - перебирает массив такой же метод перебора который принимает callback
// в качестве callback у функции будет два параметра
/*

result - переменная куда я буду накапливать результат

item - текущий элемент

для массивов помимо индексовки есть метод инcludes 

includes позволяет проверить есть в массиве данный элемент или нет, возвращает true или false

let variable_6 = array_52.reduce((result,item) => {
    return result.includes(item) ? result
})

и теперь если в result у меня такой элемент уже есть я просто возвращаю result и просто ничего не делаю
в противном случае я добавляю в result - текущий элемент и все это преобразую обратно в массив [...result, item]

let variable_6 = array_52.reduce((result,item) => {
    return result.includes(item) ? result : [...result, item]
})

чтобы это сработало и не выдало ошибок мне нужно изначально определить result как массив
иначе попытка применения includes даст ошибку поэтому я задаю согласно документации исходное значение
...
console.log(variable_6); // (9) [1, 2, 3, 9, 4, 5, 6, 7, 8]

// __________________________________ Методы массивов Every, some, find нужны ли ________________________________________

/* 

Задача: есть ли среди вложенных объектов
объект левел которого называется doomer
...
const jamesBond = {
  ...
  getFullName: function () {
    // метод getFullName который возвращает строку
...
// ___________________________ 10 сниппетов за 20 минут. Чистый код на JavaScript __________

// снипет это код который позволяет решить ту или иную задачу
/* 

Сниппет №1 округление к меньшему

let n1 = 456.67
console.log(Math.floor(n1)) - округление в нижнюю сторону- 455

но если нужно написать эту же запись коротко нужно воспользоваться следующим сниппетом

console.log(~~(n1)) - 455 - округление до меньшего


// Сниппет №2 возведение числа в степень

let n2 = 3
console.log(Math.pow(n2 - какое число, 3 - степень в которую возводим)) // - 27

и ту же операцию можно выполнить с помощью

console.log(n2**3) // - 27


// Сниппет №3 перевод из строки в число

можно сделать: 

console.log(parseInt(n3)) -3 

чтобы увидеть что тип изменился 

let n3 = '3'
console.log(typeof parseInt(n3)) // 3 - number

ту же операцию по другому
...
эти две записи не равнозначны

если у меня в переменной 

let n_3 = '3a'

то

console.log(parseInt(n_3)) // -3
console.log(+n_3) // - NaN
*/

let n1 = 455.67;
console.log(Math.floor(n1)); // - 455
console.log(~~n1); // 455

// Сниппет №2 возведение числа в степень

let n2 = 3;
console.log(Math.pow(n2, 3)); // - 27
console.log(n2 ** 3); // - 27
...
// этот блок можно сократить с помощью сниппета

n4 && console.log("№4"); // №4

// нюанс допустим у нас не true а число
...
// оба сниппета сработали то есть данный сниппет можно применять не только с true
// но и с числами строками оба будут работать но есть нюанс
// заменим n44 на false
...
n44 && console.log("false 4"); // ничего не выводит то есть с false n44 && console.log('false 4') работает не корректно
// тот же результат с если вместо false подставить 0
// то есть если результат значения может быть сведен к false
// то данная запись n44 && console.log('false 4') не работает

// Сниппет №5

// как мне перевести число 5 в строку

let n5 = 5;
console.log(n5.toString()); // 5 отличие вывода строки от числа - цвет 5 вывелось белым, строка 3 выводилось фиолетовым
// или
console.log(`${n5}`); // 5

// Сниппет №6 вызов функции по условию
...
// классическая конструкция выглядит следующим образом

let n6 = 6;
if (n6 === 6) {
  f_1(); // if n6 = 6 // - _1     if n6 = 60 // - _2
} else {
  f_3();
}

// сокращаем запись выше с помощью тернарного оператора, в этом случае обращаем внимание на скобки при вызове функции

(n6 === 2 ? f_1 : f_3)(); // - if n6 === 6 - _1  if n6 === 7 - _2

// Сниппет №7 используется для задания обязательного значения
// проверяет пришёл ли в функцию параметр

// в JS можно задать значение по умолчанию но
// иногда если значение не пришло должна быть ошибка и мы не должны дальше продолжать работу
/* 
пусть функция будет принимать параметр на входе

функция проверяет есть ли этот параметр
если его забыли указать то дальше не выполнять программу
*/
function f_7(parametr) {
...
JS_L.js:8779 Uncaught Error: problem
at f_7 (JS_L.js:8779:15)
at JS_L.js:8784:1

*/
console.log(f_7(1)); // 1

// каждый раз писать вот так функцию -неудобно поэтому

let checkParam = () => {
  throw new Error("problem");
}; // эта функция всего лишь бросает ошибку
// текст ошибки можно передавать в качестве параметра

let f_8 = (parametr = checkParam()) => {
  return parametr;
};
...
/* 
f_8()
JS_L.js:8798 Uncaught Error: problem
    at checkParam (JS_L.js:8798:11)
    at f_8 (JS_L.js:8802:24)
    at JS_L.js:8806:1
...
в ES6 можно писать function name(a = 5(по умолчанию равный 5))
и если параметр а не передадут то 5 присвоится в а

здесь тоже самое если parametr указан то он просто передается внутрь функции 
если нет ему присваивается checkParam() (как 5 в а)
а правая часть это ошибка
...
// Сниппет №8 Получение символа из строки

let n8 = "sample string";
console.log(n8.charAt(1)); // charAt(1) указываем ИНДЕКС буквы в строке. // - a
// то есть мы можем обращаться к строке как к массиву используя []
console.log(n8[0]); // s

// Сниппет №9 позволяет писать строку в несколько строк
// в JS можно писать строки в несколько строк
let n9 = `
ilia
artem
maksim
...
// Сниппет №10 значение по дефолту

// пусть у меня есть переменная в значении которой я не уверен
let n10 = 34;
// значение может прийти а может его и не быть, может его забыли присвоить
// мне нужно получить значение этой переменной в result
// либо какое-либо другое значение если изначальной переменной не будет присвоено значение
let result = n10 || false;
console.log(result); // 34 если n10 не присвоено значение то false undefined - тоже будет false

// _____________________ Ожидаем загрузку изображения JavaScript, плюс - ограничиваем скорость загрузки __________

// способ определения загрузки картинки, как узнать загрузилась ли картинка
// получаем картинки в JS

const images = document.querySelectorAll("#Img");

// создам переменную интервал и напишу новый интервал

let imageInterval;

// интервал позволяет запускать одни и те же действия через равные промежутки времени
// первым параметром идет функция () => {} a вторым параметром идет время выполнения 50 миллисекунд
// у элементов есть свойство complete оно принимает 2 значения либо true либо false
// true - если элемент загружен на страницу false если не загружен
// я буду через каждые 50 миллисекунд проверять данные свойства
// и если данное свойство станет true то надо закончить imageInterval
// иначе он будет бесконечный
// поэтому дописываем прерывание if() {то мне необходимо прекратить работу дописываем очистку интервала}
// clearInterval я должен указать какой интервал я очищаю
...
еще можно помимо complete проверять высоту картинки
*/

// _________ Переключаем части статьи на мобильном. JavaScript ____________
...
// получаем все секции в виде массива данных состоящего из секция
let sections = document.querySelectorAll(".sections");

// вычисляю размер массива состоящего из элементов в виде секций
let sectionsCount = sections.length;

// по hash будем ориентироваться где я нахожусь
/* Хэш — функция представляет собой метод или функция, которая принимает ключ по элементу в качестве входных данных, 
присваивает индекс специфичный для данного ключа и возвращает индекс всякий раз, когда ключ ищется. 
Эта операция обычно возвращает тот же хэш для данного ключа.*/
let hash = 0
// Если сайт загружается 1 раз hashа там не будет проверяем

if (window.location.hash) {
    hash = window.location.hash // переводим hash в число
    hash = +hash.replace('#','') // вытаскиваю из хэша только число то есть ту секцию на которой я нахожусь
    showSection() // перебирает все section которые есть
}

// проверяю если хэш можно наращивать
function articleLeft() {
    if (hash -1 >= 0) {
        hash--
    }
    else {
        hash = sectionsCount - 1
    }
    showSection()
}

function articleRight() {
    if (hash +1 <= sectionsCount) {
        hash++
    }
    else {
        hash = 0
...
function showSection() {
    ...
    }) // но у меня есть 1 элемент на который не нужно добавлять это тот элемент на который указывает хэш
    sections[hash].classlist.remove('hide-section')
} 

// нихера не получается

// _________ Выравниваем блок по центру. CSS. 7 приемов - способов __________

// 1 способ выравнивания по центру 

/* 
Родительскому тегу присвоить text-align(выравнимание по центру)

text-align: center; - на родителя
display: inline block; - на потомка

но для того чтобы text-align сработал вложенный блок должен быть либо inline либо inline block
то есть данное свойство действует на текст на изображения на span но на div оно не подействует
для того чтобы на div подействовал text-align center он должен быть inline 
если мы хотим ширину и высоту то display: inline block

*/

// 2 способ margin - auto

/* 

Заключается в том что мы только блоку который нам нужен (потомку) прописываем свойство родителю при этом ничего не прописываем

потомку прописываем два свойства

background: green; - чтобы видеть
margin: 10px auto; - вместо 10px может быть любое значение

margin: 10px auto; - содержит в себе:
...
auto - значит подсчитывается одинаковый отступ с разных сторон

10px - можно не задавать

можно написать marginLeft marginRight
...
/*
Когда к блоку применяется absolute он перестает влиять на размер родителя он его не растягивает из за этого родитель схлопывается и потомок оказывается
визуально за его пределами 

для родителя добавляем position: relative; чтобы позиционироватьсья относительно него 
для потомка вложенного элемента position: absolute;
*/

// 4 способ вместо margin-left написать transform: translate;(-50%,0) и переместить блок на 50% вместо margin-left

// 5 cпособ flexbox

/* для родительского элемента это все что нужно для выравнимания по центру
.flex-div {
    display: flex;
    align-items: center; без этого свойства тоже работает но если в будущем планируется расширение то понадобится и оно
